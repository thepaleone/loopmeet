...
- [X] T002 Confirm the save control cannot render blank. *(Checked the `ic_save.png` asset and its `<MauiImage>` glob; the save control then shipped as a text glyph instead, matching the edit and map affordances, so no image asset is on this path at all.)*
