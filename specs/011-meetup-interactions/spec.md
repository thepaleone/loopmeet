...
- **Icon style**: Text glyphs are used rather than image files, matching the delete and accept actions already shipping in the app, because that presentation is already proven across all supported platforms. This applies to the save control as well: it ships as a glyph rather than reusing the `ic_save.png` asset, keeping all three new affordances consistent (that asset remains in use on the Profile page).
