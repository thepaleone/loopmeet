...
- **Source inspection**: save icon on the title row and bottom button gone (both forms); card tap bound to the details command; location label's tap recognizer removed; map glyph bound to the maps command and gated on `CanOpenLocation`; details page pencil gated on `IsOwner`; `meetup-detail` route registered.
...
| 7 | US2 | Home card opens details | Home → tap a meetup card (anywhere except the map glyph) | Details screen opens for that meetup |
...
| 11 | US2 | Location TBD | View the meetup with no location | Location reads "TBD"; no map glyph on the card or the details screen |
| 12 | US2 | Map launch from details | Details screen for a located meetup → tap the map glyph | Native maps app opens at the correct place |
| 13 | US2 | Map launch from card | Home card for a located meetup → tap the card's map glyph | Native maps opens; the details screen does **not** open |
...
| 23 | Edge | Deleted while open | Open details as **Member**; have **Owner** delete that meetup; pull to refresh / re-enter | "No longer available" state; no pencil, no map control, no blank screen |
...
| 26 | FR-010 | Name without coordinates | A meetup with a place name but no coordinates (seed directly if needed) | Name shown; no map glyph on card or details |
