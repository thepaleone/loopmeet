...
public sealed partial class EditMeetupViewModel : ObservableObject
{
    ...
    public EditMeetupViewModel(MeetupsApi meetupsApi, PlacesApi placesApi)
        : this(meetupsApi, placesApi, new MeetupLocationLookupBehavior())
    {
    }

    public EditMeetupViewModel(MeetupsApi meetupsApi, PlacesApi placesApi, MeetupLocationLookupBehavior locationLookupBehavior)
    {
    ...
    {
        if (GroupId == Guid.Empty || MeetupId == Guid.Empty) return;

        try
        {
            var response = await _meetupsApi.GetGroupMeetupsAsync(GroupId);
            var meetup = response.Meetups.FirstOrDefault(m => m.Id == MeetupId);
            ...
            if (!string.IsNullOrWhiteSpace(meetup.PlaceName))
            {
                LocationDisplay = meetup.PlaceName;
                HasSelectedLocation = true;
            }
        }
        catch
        {
            ErrorMessage = "Could not load meetup details.";
        }
    }
    ...
    private async Task SaveAsync()
    {
        ...
        try
        {
            await _meetupsApi.UpdateMeetupAsync(GroupId, MeetupId, new UpdateMeetupRequest
            {
                Title = trimmedTitle,
        ...
        {
            ErrorMessage = "Could not update the meetup. Please try again.";
        }
        finally
        {
            IsBusy = false;
